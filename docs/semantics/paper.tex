...
\begin{figure}
  ...
  \begin{array}{lllllll}
    ...
    |\rho\ t\ @x\langle t_1 \rangle.T'\ \mhyph\ t'| & = & |t'| \\
...
\item The \(\rho\) construct allows one to rewrite occurrences of \(t_2\) in the
  checked type of the \(T\) of the whole expression to \(t_1\) before checking
  ...
  term \(t_1\) must be \(\beta\eta\)-convertible (modulo erasure) to \(t_1'\).
  Then, the user provided type \(T'\) is checked to have kind \(\star\) after
  replacing occurrences of \(x\) with \(t_1\), and the subexpression \(t'\) is
  checked against this type.
  Finally, \([t_2/x]T'\) (which need not be well-kinded) must be convertible
...
\interp{\Gamma}\), we have \(\sigma|t_1'| =_{\cbe} \sigma|t_1|\) (and
\(\sigma|t_1|\) is closed), so we obtain
\([\sigma|t_1|]_{\cbe} = [\sigma|t_2|]_{\cbe}\).
By the IH applied to the first premise of the second row,
\(\interp{[t_1/x]T}_{\sigma,\rho} \in \mathcal{R}\) and so is defined.
By the IH applied to the second premise in the second row,
\([\sigma|t'|]_{c\beta\eta} \in \interp{[t_1/x]T'}_{\sigma,\rho}\).
By the IH applied to the second premise in the second row,
\(\interp{T}_{\sigma,\rho} = \interp{[t_2/x]T'}_{\sigma,\rho}\).
...
\begin{proof}
  ...
  convertibility, we have \([t_2/x]T' \cong T_4\) (\(T_4\) is the type we must
...
\begin{proof}
  ...
  \startcase{.2cm}
  \[
    \infer{\Gamma_1,x_1:T_1,\Gamma_2 \vdash \rho\ t\ @x\langle t_1 \rangle.T'\ \mhyph\ t' \tpcheck T_3}
    {
      \begin{array}{ccc}
        \Gamma_1,x_1:T_1,\Gamma_2 \vdash t \tpsynthleads \{t_1' \simeq t_2\}
        & \textit{FV}(t_1) \subseteq \textit{dom}(\Gamma_1,x_1:T_1,\Gamma_2)
        & |t_1'| =_{\beta\eta} |t_1|
        \\ \Gamma_1,x_1:T_1,\Gamma_2 \vdash [t_1/x] T' \tpsynth \star
        & \Gamma_1,x_1:T_1,\Gamma_2 \vdash t \tpcheck [t_1/x]T'
        & [t_2/x]T' \cong T_3
      \end{array}
    }
  \]
...
\begin{proof}
  ...
  \startcase{.2cm}
  \[
    \infer{\Gamma_1,x:T,\Gamma_2 \vdash \rho\ t''\ @x_1\langle t_1 \rangle.T_2\ \mhyph\ t' \tpcheck T_1}
    {
      \begin{array}{ccc}
        \Gamma_1,x:T,\Gamma_2 \vdash t'' \tpsynthleads \{t_1' \simeq t_2\}
        & \textit{FV}(t_1) \subseteq \textit{dom}(\Gamma_1,x:T,\Gamma_2)
        & |t_1'| =_{\beta\eta} |t_1|
        \\ \Gamma_1,x:T,\Gamma_2 \vdash [t_1/x_1] T_2 \tpsynth \star
        & \Gamma_1,x:T,\Gamma_2 \vdash t' \tpcheck [t_1/x_1]T_2
        & [t_2/x_1]T_2 \cong T_1
      \end{array}
    }
  ...
  \simeq [t/x]t_2\}\), that \(\Gamma_1,[t/x]\Gamma_2 \vdash [t/x][t_1/x_1]T_2
  \tpsynth \star\), and that \(\Gamma_1,[t/x]\Gamma_2 \vdash [t/x]t' \tpcheck
  [t/x][t_1/x_1]T_2\).
  From the last premise, we may conclude that \([t/x]T_1 \cong
  [t/x][t_2/x_1]T_1\).
  We also see from the premises that \(\textit{FV}([t/x]t_1) \subseteq
  \textit{dom}(\Gamma_1,[t/x]\Gamma_2)\) and that \(|[t/x]t_1'| =_{\beta\eta}
  |[t/x]t_1|\).
  Applying the rule and permuting substitutions gives us the desired result
  (note for example that \([t/x][t_1/x_1]T_1 = [[t/x]t_1/x_1][t/x]T_1\)).
\end{proof}
