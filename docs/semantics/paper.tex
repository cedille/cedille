...
\begin{proof}
  ...
  \startcase{.2cm}
  \[
    \infer{\Gamma_1,X_1:\kappa_1,\Gamma_2 \vdash \rho\ t'\ @x\langle t_2 \rangle.T\ \mhyph\ t \tpsynth [t_2/x]\ T}
    {
      \begin{array}{ccc}
  ...
  By the IH on the second premise of the second row,
  \(\Gamma_1,X_1:\kappa_1',\Gamma_2 \vdash [t_2/x]\ T \tpsynth \star\)
  (\(\star\) is only convertible with itself).
  We apply the IH to the second premise of the second row and use the rule to conclude
  (\(\textit{dom}(\Gamma_1,X_1:\kappa_1,\Gamma_2) =
  \textit{dom}(\Gamma_1,X_1:\kappa_1',\Gamma_2)\)).
\end{proof}
...
\begin{proof}
  ...
  \startcase{.2cm}
  \[
    \infer{\Gamma_1,x_1:T_1,\Gamma_2 \vdash \varphi\ t\ \mhyph\ t'\ \{t''\} \tpcheck T_3}
    {
      \begin{array}{ccc}
        \Gamma_1,x_1:T_1,\Gamma_2 \vdash t\tpsynthleads \{t_1\simeq t_2\}
        &
        & \Gamma_1,x_1:T_1,\Gamma_2\vdash t' \tpcheck T_3
        \\ |t_1| =_{\beta\eta} |t'|
        & |t_2| =_{\beta\eta} |t''|
  ...
  t \tpsynth T\) (with no greater depth) for some \(T \cong \{t_1 \simeq t_2\}\).
  This means that \(T \leadsto_\beta^* \{t_3 \simeq t_4\}\) for some \(t_3,t_4\)
  such that \(|t_3| =_{\beta\eta} |t_1|\) and \(|t_4| =_{\beta\eta} |t_2|\).
  Using the first and second premise of the second row and transitivity of
  \(\beta\eta\)-equivalence, \(|t_3| =_{\beta\eta} |t'|\) and \(|t_4|
  =_{\beta\eta} |t''|\).
  By the IH on the second premise of the first row, \(\Gamma_1,x_1:T_2,\Gamma_2
  \vdash t' \tpcheck T_4\) (with no greater depth, and where \(T_4\) is the type
  given to us in the lemma and \(T_4 \cong T_3\)).
  Also, we can see from the last premise of the second row that
  \(\textit{FV}(t') \subseteq \textit{dom}(\Gamma_1,x_1:T_2,\Gamma_2)\).
  So, we use the rule to conclude \(\Gamma_1,x_1:T_2,\Gamma_2 \vdash \varphi\ t\
  \mhyph\ t'\ \{t''\} \tpcheck T_4\).
  ...
  \startcase{.2cm}
  \[
    \infer{\Gamma_1,x_1:T_1,\Gamma_2 \vdash \rho\ t'\ @x\langle t_2 \rangle.T\ \mhyph\ t \tpsynth [t_2/x]\ T}
    {
      \begin{array}{ccc}
        \Gamma_1,x_1:T_1,\Gamma_2 \vdash t' \tpsynthleads \{t_1 \simeq t_2'\}
        & \textit{FV}(t_2) \subseteq \textit{dom}(\Gamma)
  ...
  \]
  By the IH on the first premise, \(\Gamma_1,x_1:T_2,\Gamma_2 \vdash t \tpsynth
  T_3\) (with no greater depth) for some \(T_3 \cong \{t_1 \simeq t_2'\}\).
  This means \(T_3 \leadsto_\beta^* \{t_3 \simeq t_4\}\) for some \(t_3,t_4\)
  such that \(|t_3| =_{\beta\eta} t_1\) and \(|t_4| =_{\beta\eta} |t_2'|\).
  So we further obtain \(|t_4| =_{\beta\eta} |t_2|'\).
  By the IH on the first premise of the second row, \(\Gamma_1,x_1:T_2,\Gamma_2
  \vdash [t_2/x]T \tpsynth \star\) with no greater depth.
  By the IH on the second premise of the second row, \(\Gamma_1,x_1:T_2,\Gamma_2
  \vdash t \tpsynth T''\) (with no greater depth) for some \(T'' \cong T'\).
  With the final premise of the second row and transitivity, we obtain \(T''
  \cong [t_1/x]T\).
  ...
  \startcase{.2cm}
  \[
    \infer{\Gamma_1,x_1:T_1,\Gamma_2\vdash \varphi\ t\ \mhyph\ t'\ \{t''\} \tpsynth T}
    {
      \begin{array}{ccc}
        \Gamma_1,x_1:T_2,\Gamma_2\vdash t\tpsynthleads \{t_1\simeq t_2\}
        &
        & \Gamma_1,x_1:T_1,\Gamma_2\vdash t' \tpsynth T
        \\ |t_1| =_{\beta\eta} |t'|
        & |t_2| =_{\beta\eta} |t''|
  ...
  Similar to the checking case for \(\varphi\), only now after using the IH the
  second premise of the first row synthesizes a type \(T' \cong T\).
  The whole expression, then, synthesizes type \(T'\).
\end{proof}
