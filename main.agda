...
processFile : (dir : string) → (file : string) → toplevel-state → IO (𝔹 × toplevel-state) -- the boolean is for if there was an erro
...
process-cmd dir (DefKind pi x _ k pi') (mk-toplevel-state (mk-include-state is) Γ ss) = 
  let ss' = (check-kind Γ k ≫span
             ...
             spanMok) ss in
  return (mk-toplevel-state (mk-include-state is) (ctxt-kind-def x (hnf Γ unfold-head k) Γ) (snd ss'))
...
processFile dir file s | (mk-toplevel-state (mk-include-state is) Γ ss) | input-filename = 
  ...
  if b then
    ...
    return (tt , mk-toplevel-state (mk-include-state is) Γ
  ...
  where processText : string → IO (𝔹 × toplevel-state)
        ...
        processText x | inj₂ r | _ = return (tt , mk-toplevel-state (mk-include-state is) Γ
...
processArgs (input-filename :: []) = 
  ...
  where finish : 𝔹 × toplevel-state → IO ⊤
        finish (b , mk-toplevel-state (mk-include-state is) Γ ss) = 
