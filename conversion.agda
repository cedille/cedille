...
conv-term Γ t t' = ff
conv-type Γ t t' = ff
conv-kind Γ k' (KndParens k) = conv-kind Γ k' k
conv-kind Γ (KndParens k) k' = conv-kind Γ k k'
...
conv-kind Γ (KndArrow k k₁) (KndPi x (Tkk k') k'') = conv-kind Γ k k' && conv-kind Γ k₁ k''
conv-kind Γ (KndArrow k k₁) _ = ff
conv-kind Γ (KndPi x (Tkk k₁) k) (KndArrow k' k'') = conv-kind Γ k₁ k' && conv-kind Γ k k''
conv-kind Γ (KndPi x (Tkk k₁) k) (KndPi x' (Tkk k') k'') = 
  let Γ' = ctxt-type-def Γ x (TpVar posinfo-gen x') k₁ in
    conv-kind Γ k₁ k' && conv-kind Γ' k k''
conv-kind Γ (KndPi x (Tkt t) k) (KndTpArrow t' k'') = conv-type Γ t t' && conv-kind Γ k k''
conv-kind Γ (KndPi x (Tkt t) k) (KndPi x' (Tkt t') k') = 
  let Γ' = ctxt-term-def Γ x (Var posinfo-gen x') t in
    conv-type Γ t t' && conv-kind Γ' k k'
conv-kind Γ (KndPi x (Tkt t) k) _ = ff
conv-kind Γ (KndPi x (Tkk k') k) _ = ff
conv-kind Γ (KndTpArrow t k) (KndTpArrow t' k') = conv-type Γ t t' && conv-kind Γ k k'
conv-kind Γ (KndTpArrow t k) (KndPi x (Tkt t') k') = conv-type Γ t t' && conv-kind Γ k k'
