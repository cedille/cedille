...
synth-type s (Δ , b , r) (Eprint c e) t = 
  synth-type s (Δ , b , r) e t ≫synth λ k → 
  no-error ((show-evctxt-if c (Δ , b , r) ^ type-to-string r t ^ " ⇒ " ^ kind-to-string r k ^ "\n") , k)
synth-type s (Δ , b , r) EholeSilent t = holewrong-type r t
synth-type s (Δ , b , r) (Ehole _) t = holewrong-type r t
synth-type s (Δ , b , r) (EholeNamed _ _) t = holewrong-type r t
...
synth-type s (Δ , b , r) (Evar u) t | nothing | just (t' , k) = 
  if eq-type s (bctxt-contains b) r t t' then (no-error ("" , k)) 
                 ...
                 ^ "1. the evidence variable: " ^ u ^ "\n"
                 ^ "2. the kinding it proves: " ^ type-to-string r t' ^ " ∷ " ^ kind-to-string r k ^ "\n"
                 ^ "3. the type we are trying to kind: " ^ type-to-string r t))
synth-type s (Δ , b , r) (Evar u) t | just (term-type trm tp) = 
  yes-error ("An evidence variable proving a typing is being used to try to synthesize a kind.\n"
           ^ "1. the evidence variable: " ^ u ^ " ∷ " ^ term-to-string r trm ^ " : " ^ type-to-string r tp ^ "\n"
           ^ "2. " ^ synth-type-errstr r t)
...
synth-type s (Δ , b , r) (Evar u) t | just (type-kind t' k) | ff = 
  yes-error ("An evidence variable is being used to try to synthesize a kind, but that variable proves a different kinding.\n"
           ^ "1. the evidence variable: " ^ u ^ " ∷ " ^ type-to-string r t' ^ " : " ^ kind-to-string r k ^ "\n"
           ^ "2. " ^ synth-type-errstr r t)
synth-type s (Δ , b , r) (Evar u) t | just (ev-ctorset Θ) = 
  yes-error ("An evidence variable proving a ctor set is being used to try to synthesize a kind.\n"
           ^ "1. the evidence variable: " ^ u ^ " ∷ " ^ ctorset-to-string r Θ ^ "\n"
           ^ "2. " ^ synth-type-errstr r t)

synth-type s (Δ , b , r) Check (TpApp t t') = synth-type s (Δ , b , r) (Eapp Check Check) (TpApp t t')
  ...
  where h : kind → synth-t kind
        ...
        h (KndVar x) | nothing =
          yes-error ("An undeclared kind variable was synthesized for the first part of a type-level application.\n"
                   ^ "1. the first part of the application: " ^ type-to-string r t ^ "\n"
        ...
        h k = yes-error ("We synthesized a non-functional kind for the first part of a type-level application to a type.\n"
                       ^ "1. the first part of the application: " ^ type-to-string r t ^ "\n"
                       ^ "2. the synthesized kind: " ^ kind-to-string r k)

synth-type s (Δ , b , r) Check (TpAppt t t') = synth-type s (Δ , b , r) (Eapp Check Check) (TpAppt t t')
...
synth-type s (Δ , b , r) Check (TpVar x) = synth-type s (Δ , b , r) (Evar x) (TpVar x)
synth-type s (Δ , b , r) e (TpVar x) with lookup-type-var s (renamectxt-rep r x)
synth-type s (Δ , b , r) e (TpVar x) | just tp = synth-type s (Δ , b , r) e tp
synth-type s (Δ , b , r) e (TpVar x) | nothing = 
  yes-error ("We cannot synthesize a kind for the given type variable (not defined), with the given evidence.\n"
            ^ "1. the type variable: " ^ x ^ "\n"
            ^ "2. the evidence: " ^ evidence-to-string r e)

synth-type s (Δ , b , r) e t = 
...
try-synth-type s (Δ , b , r) e t k = 
  ...
  if eq-kind s (bctxt-contains b) r k k' then no-error "" 
                ...
                ^ "2. the kind we synthesized for it: " ^ kind-to-string r k' ^ "\n"
                ^ "3. the kind we are checking against: " ^ kind-to-string r k))
  
synth-term s Γ (Eparens e) trm = synth-term s Γ e trm
synth-term s Γ e (Parens trm) = synth-term s Γ e trm
synth-term s (Δ , b , r) (Eprint c e) trm = 
  synth-term s (Δ , b , r) e trm ≫synth λ tp →
  no-error ((show-evctxt-if c (Δ , b , r) ^ term-to-string r trm ^ " ⇒ " ^ type-to-string r tp ^ "\n") , tp)
synth-term s (Δ , b , r) EholeSilent trm = holewrong-term r trm
synth-term s (Δ , b , r) (Ehole _) trm = holewrong-term r trm
synth-term s (Δ , b , r) (EholeNamed _ _) trm = holewrong-term r trm
...
synth-term s (Δ , b , r) Check (Var x) = synth-term s (Δ , b , r) (Evar x) (Var x)
...
synth-term s (Δ , b , r) (Evar u) t | just (ev-ctorset Θ) | nothing = 
  yes-error ("An evidence variable proving a ctor set is being used to synthesize a type from a term,\n"
           ...
           ^ "1. " ^ synth-term-errstr r t ^ "\n"
           ^ "2. the evidence variable: " ^ u ^ "\n"
           ^ "3. the ctor set the evidence variable proves: " ^ ctorset-to-string r Θ)
  ...
  where h : type → synth-t type
        ...
        h (TpVar x) | nothing =
          yes-error ("An undeclared type variable was synthesized for the first part of an instantiation.\n"
                   ^ "1. the first part of the instantiation " ^ term-to-string r t ^ "\n"
        ...
        h tp = yes-error ("We do not have a matching case for the instantiation value and synthesized type"
                         ^ " for an instantation.\n"
                       ^ "1. the first part of the instantiation " ^ term-to-string r t ^ "\n"
                       ^ "2. the synthesized type: " ^ type-to-string r tp ^ "\n"
                       ^ "3. the instantiation value: " ^ term-to-string r t')
...
synth-term s (Δ , b , r) (Eapp e (Eappk e' tp)) t =
  ...
  where h : type → synth-t type
        ...
        h tp = yes-error ("We do not have a matching case for the instantiation value and synthesized type"
                       ...
                       ^ "1. the first part of the instantiation " ^ term-to-string r t ^ "\n"
                       ^ "2. the synthesized type: " ^ type-to-string r tp ^ "\n"
                       ^ "3. the instantiation value: " ^ type-to-string r tp)



synth-term s (Δ , b , r) Check (App t t') = synth-term s (Δ , b , r) (Eapp Check Check) (App t t')
  ...
  where h : type → synth-t type
        ...
        h (TpVar x) | nothing =
          yes-error ("An undeclared type variable was synthesized for the first part of an application.\n"
                   ^ "1. the first part of the application: " ^ term-to-string r t ^ "\n"
                   ^ "2. the synthesized type variable: " ^ x)
        h (TpVar x) | just tp' = h tp'
...
synth-term s (Δ , b , r) (Rbeta e t' e') t | just t1 , m =
  if eq-term s (bctxt-contains b) r t t1 then
    ...
    yes-error (m ^ "\nIn a reverse-beta proof, we were able to convert the given term, but the result is not equal to the term we are\n"
             ...
             ^ "1. the term we converted to: " ^ term-to-string r t1 ^ "\n"
             ^ "2. " ^ synth-term-errstr r t)               
...
synth-term s (Δ , b , r) (Proj e i) t = 
  ...
  where h : index → type → synth-t type
        ...
        h i tp = yes-error ("We synthesized a type which is not a iota-type, when synthesizing with a projection\n"
                       ^ "as the evidence.\n"
                       ^ "1. the term for which we synthesized a type: " ^ term-to-string r t ^ "\n"
                       ^ "2. the synthesized type: " ^ type-to-string r tp)

synth-term s Γ (Ctora x) trm with lookup-type-var s x
synth-term s (Δ , b , r) (Ctora x) trm | nothing =
  ...
  where h : type → synth-t type
        ...
        h (Nu X k Θ tp) | nothing = 
          yes-error ("Constructor evidence (ζ-expression) is being used, but we cannot find the term in question in the "
                   ^ "constructor set.\n"
                   ^ "1. the type whose kinding has been established: " ^ type-to-string r (Nu X k Θ tp) ^ "\n"
                   ^ "2. " ^ synth-term-errstr r trm)
        ...
        h t = 
          yes-error ("Constructor evidence (ζ-expression) is being used, but the subevidence given does not show kinding of a nu-type.\n"
                   ^ "1. the type proved: " ^ type-to-string r t)
        

synth-term s (Δ , b , r) e trm = 
  yes-error ("We have no matching case for synthesizing a type for the given term from the given evidence.\n"
...
try-synth-term s (Δ , b , r) e trm tp = 
  ...
  if eq-type s (bctxt-contains b) r tp tp' then no-error ""
                ...
                ^ "1. the term we are checking: " ^ term-to-string r trm ^ "\n"
                ^ "2. the type we synthesized for it: " ^ type-to-string r tp' ^ "\n"
                ^ "3. the type we are checking against: " ^ type-to-string r tp)
