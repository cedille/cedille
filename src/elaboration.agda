  ...
  elab-mu Γ x+e t Tₘ? ms T? =
    ...
    case decompose-tpapps Tₜ of λ where
      (TpVar _ X , as) →
        ...
        elab-optType (either-else' x+e (λ x → ctxt-type-decl' pi-gen (mu-Type/ x) (ctxt-datatype-info.k d) Γ) λ _ → Γ) Tₘ? ≫=maybe λ Tₘ? →
...
elab-cmds q? ts ρ φ μ ((DefDatatype (Datatype pi pi' x ps k dcs) pi'') :: cs) =
  ...
  maybe-else (just (cs' ++ cs'' , (record ts {Γ = set-ps (toplevel-state.Γ ts) $ ctxt-get-current-params Γ}) , ρ , φ , μ)) just $
  ...
  maybe-else (just (cs' ++ cs'' ++ cs , ts , ρ , φ , μ)) just $
