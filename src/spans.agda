...
spanM-push-term-def pi x t T Γ ss = let qi = ctxt-get-qi Γ x in returnM ((qi , qi ≫=maybe λ qi → ctxt-get-info (fst qi) Γ) , ctxt-term-def pi localScope tt x (just t) T Γ , ss)

spanM-push-term-udef : posinfo → var → term → spanM restore-def
spanM-push-term-udef pi x t Γ ss = let qi = ctxt-get-qi Γ x in returnM ((qi , qi ≫=maybe λ qi → ctxt-get-info (fst qi) Γ) , ctxt-term-udef pi localScope tt x t Γ , ss)
...
spanM-push-type-def pi x t T Γ ss = let qi = ctxt-get-qi Γ x in returnM ((qi , qi ≫=maybe λ qi → ctxt-get-info (fst qi) Γ) , ctxt-type-def pi localScope tt x (just t) T Γ , ss)
...
check-for-type-mismatch Γ s tp tp' =
  let tp'' = hnf Γ unfold-head tp' in
...
arg-argument Γ (TmArg me x) = term-argument Γ x
arg-argument Γ (TpArg x) = type-argument Γ x
...
Decl-span : ctxt → decl-class → posinfo → posinfo → var → tpkd → erased? → posinfo → span
...
TpQuant-span : ctxt → TpQuant-e → posinfo → posinfo → var → tpkd → type → checking-mode → 𝕃 tagged-val → err-m → span
...
TpLambda-span : ctxt → posinfo → posinfo → var → tpkd → type → checking-mode → 𝕃 tagged-val → err-m → span
...
KndPi-span : ctxt → posinfo → posinfo → var → tpkd → kind → checking-mode → err-m → span
...
Lam-span : ctxt → checking-mode → posinfo → posinfo → erased? → var → tpkd → term → 𝕃 tagged-val → err-m → span
...
...| is-free | tₒ with erase tₒ | hnf Γ unfold-all tₒ ff
...
Epsilon-span : posinfo → left-right → maybeMinus → term → checking-mode → 𝕃 tagged-val → err-m → span
Epsilon-span pi lr m t check tvs = mk-span "Epsilon" pi (term-end-pos t) 
  ...
  where side : left-right → string
        side (just ff) = "the left-hand side"
        side (just tt) = "the right-hand side"
        side nothing = "both sides"
...
optGuide-spans : maybe ExGuide → checking-mode → spanM ⊤
optGuide-spans nothing _ = spanMok
optGuide-spans (just (ExGuide pi x tp)) expected =
  get-ctxt λ Γ → spanM-add (Var-span Γ pi x expected [] nothing)

Rho-span : posinfo → term → term → checking-mode → 𝔹 → ℕ ⊎ var → 𝕃 tagged-val → err-m → span
...
Chi-span : ctxt → posinfo → maybe type → term → checking-mode → 𝕃 tagged-val → err-m → span
...
Delta-span : ctxt → posinfo → maybe type → term → checking-mode → 𝕃 tagged-val → err-m → span
...
Theta-span : ctxt → posinfo → theta → term → 𝕃 lterm → checking-mode → 𝕃 tagged-val → err-m → span
...
pattern-span pi x as = mk-span "Pattern" pi (snd $ foldr (λ a r → if fst r then r else (tt , (case a of λ {(ExCaseArg pi me x) → posinfo-plus-str pi x}))) (ff , posinfo-plus-str pi x) as) [] nothing
...
pattern-ctr-span Γ pi x as tp =
  ...
  rename-to-args ρ (CaseArg e x :: as) (Param pi pi' me x' atk pi'' :: ps) =
    Param pi pi' me x (subst-renamectxt Γ ρ atk) pi'' ::
...
--      rename-to-args (renamectxt-insert ρ x' x) as ps
  rename-to-args ρ [] (Param pi pi' me x atk pi'' :: ps) =
    Param pi pi' me x (subst-renamectxt Γ ρ atk) pi'' ::
...
Lft-span Γ pi pi' X t check tvs = mk-span "Lift type" pi (term-end-pos t) (checking-data check :: ll-data-type :: binder-data Γ pi' X (Tkk KdStar) tt nothing (term-start-pos t) (term-end-pos t) :: tvs)
...
Let-span : ctxt → checking-mode → posinfo → posinfo → erased? → var → (atk : tpkd) → (if tk-is-type atk then term else type) → term → 𝕃 tagged-val → err-m → span
...
TpLet-span : ctxt → checking-mode → posinfo → posinfo → var → (atk : tpkd) → (if tk-is-type atk then term else type) → type → 𝕃 tagged-val → err-m → span
