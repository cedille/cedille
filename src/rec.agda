...
rec-check-and-add-decl : decl-class → decl → spanM ⊤
rec-check-and-add-decl dc (Decl pi x atk pi') = 
  check-tk atk ≫span 
  spanM-add (Decl-span dc pi x atk pi') ≫span
  get-ctxt (λ Γ →
   set-ctxt (ctxt-tk-decl pi x atk Γ))
...
rec-check-and-add-decls : decl-class → decls → spanM ⊤
rec-check-and-add-decls dc (DeclsCons d ds)  = 
  rec-check-and-add-decl dc d ≫span rec-check-and-add-decls dc ds 
rec-check-and-add-decls dc (DeclsNil _) = spanMok
...
rec-check-and-add-ctor : ctxt → ctordecl → spanM ⊤
rec-check-and-add-ctor Γ (Ctordecl pi x tp) = 
  get-ctxt (λ Γ' →
    set-ctxt Γ ≫span
    check-type tp (just star) ≫span
    set-ctxt (ctxt-term-decl pi x tp Γ'))

{- check the types of all the ctors with respect to the first ctxt and
   then add declarations for them to the second ctxt.  -}
rec-check-and-add-ctors : ctxt → ctordecls → spanM ⊤
rec-check-and-add-ctors Γ (Ctordeclse _) = spanMok
rec-check-and-add-ctors Γ (Ctordeclsne (CtordeclsneNext c cs)) = 
  rec-check-and-add-ctor Γ c ≫span rec-check-and-add-ctors Γ (Ctordeclsne cs)
rec-check-and-add-ctors Γ (Ctordeclsne (CtordeclsneStart c)) = rec-check-and-add-ctor Γ c
...
rec-check-and-add-ctor-def : (no-need-to-check : 𝔹) → 
                             ctxt → string → type → decls → ctordeclsne → ℕ → ctordecl → udef → spanM ⊤
rec-check-and-add-ctor-def no-need-to-check Γ name rectp params ctors whichdecl (Ctordecl pi x tp) (Udef pi' x' t) =
 ...
 let t' = erase-term t in -- do not lambda-bind the params for t, because they just get erased
  ...
  (if no-need-to-check then
   ...
   (spanM-add (Ctordecl-span pi x tp []) ≫span
    ...
    set-ctxt (ctxt-term-def pi x t' tp' Γ'))

-- see comment for rec-check-and-add-ctor-defs below
rec-check-and-add-ctor-defs-ne : (no-need-to-check : 𝔹) → 
                                 ctxt → string → type → decls → ℕ → ctordeclsne → udefsne → spanM ⊤
rec-check-and-add-ctor-defs-ne no-need-to-check Γ name rectp params whichdecl (CtordeclsneStart c) (UdefsneStart u) = 
  rec-check-and-add-ctor-def no-need-to-check Γ name rectp params (CtordeclsneStart c) whichdecl c u
rec-check-and-add-ctor-defs-ne no-need-to-check Γ name rectp params whichdecl (CtordeclsneNext c cs) (UdefsneNext u us) = 
  rec-check-and-add-ctor-def no-need-to-check Γ name rectp params (CtordeclsneNext c cs) whichdecl c u ≫span
  rec-check-and-add-ctor-defs-ne no-need-to-check Γ name rectp params (suc whichdecl) cs us
rec-check-and-add-ctor-defs-ne _ Γ name rectp params whichdecl (CtordeclsneNext c cs) (UdefsneStart (Udef pi x t)) = 
  spanM-add (Udef-span pi x (term-end-pos t) (erase-term t)
...
rec-check-and-add-ctor-defs-ne _ Γ name rectp params whichdecl (CtordeclsneStart (Ctordecl pi x tp)) (UdefsneNext u us) = 
...
{- add the ctors with their definitions and types to the final ctxt
   ...
   ctordecls. -}
rec-check-and-add-ctor-defs : (no-need-to-check : 𝔹) → ctxt → string → type → decls → ctordecls → udefs → spanM ⊤
rec-check-and-add-ctor-defs _ Γ name rectp params (Ctordeclse _) (Udefse _) = spanMok
rec-check-and-add-ctor-defs no-need-to-check Γ name rectp params (Ctordeclsne cs) (Udefsne us) = 
  rec-check-and-add-ctor-defs-ne no-need-to-check Γ name rectp params 0 cs us
rec-check-and-add-ctor-defs _ Γ name rectp params (Ctordeclsne cs) (Udefse pi) = 
...
rec-check-and-add-ctor-defs _ Γ name rectp params (Ctordeclse pi) (Udefsne _) = 
...
process-rec-cmd : (no-need-to-check : 𝔹) → 
                  posinfo → posinfo → var → decls → indices → ctordecls → type → udefs → posinfo → spanM ⊤
process-rec-cmd no-need-to-check pi pi'' name params inds ctors body us pi' = 
  get-ctxt (λ Γ → 
   ...
   let body2 = let body' = subst-type Γ rectp name body in
    ...
    (if no-need-to-check then
      ...
      (rec-check-and-add-decls param params ≫span
       get-ctxt (λ Γp → 

        -- check the indices, in the ctxt containing the params
        rec-check-and-add-decls index inds ≫span 
     ...
     ≫span 
        ...
        set-ctxt Γ' ≫span
        (rec-check-and-add-ctor-defs tt {- do not check -} Γ' name rectp params ctors us ≫span
         -- get the modified context and pass that out to be bound as Γctors
         (get-ctxt (λ Γ → spanMr Γ))) ≫=spand λ Γctors → 

        set-ctxt Γ ≫span
        rec-check-and-add-ctor-defs no-need-to-check Γctors name rectp params ctors us ≫span
        get-ctxt (λ Γfinal →
         set-ctxt (ctxt-rec-def pi'' name body2 k2 Γfinal)))
