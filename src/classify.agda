...
check-term-app-erased-error : ctxt → checking-mode → maybeErased → term → term → type → spanM (maybe type)
check-term-app-erased-error Γ c m t t' head-tp =
...
check-term _ subject (just tp) =
  get-ctxt (λ Γ → 
    check-termi Γ subject (just (if is-intro-form subject then (hnf-instantiate-iota Γ subject tp ff) else (hnf Γ unfold-head tp tt))))

check-type Γ subject nothing = check-typei Γ subject nothing
check-type _ subject (just k) = get-ctxt (λ Γ → check-typei Γ subject (just (hnf Γ unfold-head k tt)))

check-termi-h : ctxt → posinfo → lam → posinfo → var → optClass → term → type → maybe abs → spanM ⊤
check-termi-h Γ  pi l pi' x oc t tp (just (mk-abs pi'' b pi''' x' atk _ tp')) =
  check-oc oc ≫span
  ...
  get-ctxt (λ Γ → 
    ...
    check-term Γ t (just (rename-type Γ x' x (tk-is-type atk) tp')) ≫span
  ...
  where this-span : ctxt → tk → optClass → 𝕃 tagged-val → span
        ...
        check-erasures KeptLambda All = error-data ("The expected type is a ∀-abstraction (indicating implicit input), but"
                                     ...
                                     :: [ expected-type Γ tp ]
check-termi-h Γ pi l pi' x oc t tp nothing =
   spanM-add (punctuation-span "Lambda"  pi (posinfo-plus pi 1)) ≫span
...
check-termi _ (Unfold pi t) mtp =
  get-ctxt (cont mtp)
  where cont : (mtp : maybe type) → ctxt → spanM (check-ret mtp)
        cont nothing    Γ = check-term Γ t nothing ≫=span cont'
          where cont' : (mtp : maybe type) → spanM (maybe type)
                cont' nothing = spanMr nothing
                cont' (just tp') = check-termi-return Γ t tp'
        cont (just tp') Γ = check-term Γ t (just tp')
        
check-termi Γ  (AppTp t tp') tp =
  check-term Γ t nothing ≫=span cont'' ≫=spanr cont' tp 
...
check-termi Γ (App t m t') tp =
  ...
  where cont : maybeErased → type → spanM (maybe type)
        ...
        cont Erased (TpArrow tp1 UnerasedArrow  tp2) = 
          check-term-app-erased-error Γ (maybe-to-checking tp) Erased t t' (TpArrow tp1 UnerasedArrow tp2)
        cont NotErased (TpArrow tp1 ErasedArrow tp2) = 
          check-term-app-erased-error Γ (maybe-to-checking tp) NotErased t t' (TpArrow tp1 ErasedArrow tp2)
        cont m (Abs pi b pi' x (Tkt tp1) tp2) = 
          if check-term-app-matching-erasures m b then
            ...
            check-term-app-erased-error Γ (maybe-to-checking tp) m t t' (Abs pi b pi' x (Tkt tp1) tp2)
...
check-termi Γ (Lam pi l pi' x (SomeClass atk) t) nothing =
  ...
  where cont : maybe type → spanM (maybe type)
        cont nothing = spanM-add (Lam-span Γ pi l x (SomeClass atk) t []) ≫span 
                       spanMr nothing
        cont (just tp) = 
...
check-termi Γ (Lam pi l _ x NoClass t) nothing =
            spanM-add (punctuation-span "Lambda" pi (posinfo-plus pi 1)) ≫span
  spanM-add (Lam-span Γ pi l x NoClass t [ error-data ("We are not checking this abstraction against a type, so a classifier must be"
                                                  ^ " given for the bound variable " ^ x) ]) ≫span
  spanMr nothing

check-termi Γ (Lam pi l pi' x oc t) (just tp) = check-termi-h Γ pi l pi' x oc t tp (to-abs tp)

{-
check-termi _ (Lam pi l pi' x oc t) (just tp) with to-abs tp 
check-termi _ (Lam pi l pi' x oc t) (just tp) | just (mk-abs pi'' b pi''' x' atk _ tp') =
  check-oc oc ≫span
  spanM-add (punctuation-span "Lambda" pi (posinfo-plus pi 1)) ≫span
  get-ctxt (λ Γ → 
    spanM-add (this-span Γ atk oc (check-erasures l b)) ≫span
...
check-termi _ (Beta pi ot) (just (TpEq t1 t2)) = 
...
check-termi Γ (Delta pi t) (just tp) = 
  ...
  where cont : string → maybe type → spanM ⊤
        ...
        cont errmsg nothing = spanM-add (Delta-span pi t checking [ expected-type Γ tp ])

check-termi Γ (PiInj pi n t) mtp = 
  check-term Γ t nothing ≫=span (λ tm → get-ctxt (λ Γ → cont Γ mtp tm))
  where cont : ctxt → (mtp : maybe type) → maybe type → spanM (check-ret mtp)
        ...
        cont _ mtp (just tp) =
           ...
           check-fail mtp
        cont _ mtp nothing = spanM-add (PiInj-span pi n t (maybe-to-checking mtp) (expected-type-if Γ mtp [])) ≫span check-fail mtp

check-termi Γ (Omega pi t) mtp =
...
check-termi Γ (Rho pi r t t') (just tp) = 
  check-term Γ t nothing ≫=span cont
  where cont : maybe type → spanM ⊤
        cont nothing = spanM-add (Rho-span pi t t' checking r 0 [ expected-type Γ tp ]) ≫span check-term Γ t' (just tp)
        cont (just (TpEq t1 t2)) = 
           get-ctxt (λ Γ →
             let s = rewrite-type Γ empty-renamectxt (is-rho-plus r) t1 t2 tp in
             check-term Γ t' (just (fst s)) ≫span
...
check-termi Γ (Rho pi r t t') nothing = 
  check-term Γ t nothing ≫=span λ mtp → 
  check-term Γ t' nothing ≫=span cont mtp
...
check-termi Γ (Chi pi (Atype tp) t) mtp = 
  ...
  where cont : (m : maybe type) → spanM (check-ret m)
        cont nothing = spanM-add (Chi-span Γ pi (Atype tp) t synthesizing []) ≫span spanMr (just tp)
        cont (just tp') =
          get-ctxt (λ Γ → 
...
check-termi Γ (Chi pi NoAtype t) (just tp) = 
  ...
  where cont : (m : maybe type) → spanM ⊤
        cont nothing = spanM-add (Chi-span Γ pi NoAtype t checking []) ≫span spanMok
        cont (just tp') =
          get-ctxt (λ Γ → 
...
check-termi Γ (Theta pi (AbstractVars vs) t ls) (just tp) =
...
check-termi Γ (IotaPair pi t1 t2 ot pi') (just (IotaEx pi1 Iota pi2 x (SomeType tp1) tp2)) =
  ...
  get-ctxt (λ Γ → 
    check-term Γ t2 (just (subst-type Γ t1 x tp2)) ≫span
  ...
  where err : ctxt → string → term → tagged-val
        ...
        add-spans-if (SomeTerm t _) t1 t2 = check-termi Γ t (just (TpEq (erase-term t1) (erase-term t2)))
...
check-termi Γ t tp = spanM-add (unimplemented-term-span Γ (term-start-pos t) (term-end-pos t) tp) ≫span unimplemented-if tp


check-typei-h : ctxt → posinfo → posinfo → var → tk → type → kind → (maybe absk) → spanM ⊤
check-typei-h Γ pi pi' x atk body k (just (mk-absk pik pik' x' atk' _ k')) =
   check-tk Γ atk ≫span
   ...
   add-tk pi' x atk ≫=span λ mi → 
   check-type Γ body (just (rename-kind Γ x' x (tk-is-type atk') k')) ≫span
   spanM-restore-info x mi)
check-typei-h Γ pi pi' x atk body k nothing = 
   check-tk Γ atk ≫span
   spanM-add (punctuation-span "Lambda (type)" pi (posinfo-plus pi 1)) ≫span
...
check-typei Γ (TpParens pi t pi') k =
  ...
  check-type Γ t k
check-typei Γ (NoSpans t _) k = check-type Γ t k ≫=spand spanMr
check-typei _ (TpVar pi x) mk =
  get-ctxt (cont mk)
...
check-typei Γ (TpLambda pi pi' x atk body) (just k) = check-typei-h Γ pi pi' x atk body k (to-absk k)
{-check-typei _ (TpLambda pi pi' x atk body) (just k) with to-absk k 
check-typei _ (TpLambda pi pi' x atk body) (just k) | just (mk-absk pik pik' x' atk' _ k') =
   check-tk atk ≫span
   spanM-add (punctuation-span "Lambda (type)" pi (posinfo-plus pi 1)) ≫span
   get-ctxt (λ Γ → 
   spanM-add (if conv-tk Γ atk atk' then
                ...
                TpLambda-span pi x atk body checking (lambda-bound-var-conv-error Γ x atk' atk [ kind-data k ])) ≫span
   add-tk pi' x atk ≫=span λ mi → 
...
check-typei Γ (TpLambda pi pi' x atk body) nothing =
  ...
  check-tk Γ atk ≫span
  add-tk pi' x atk ≫=span λ mi → 
  check-type Γ body nothing ≫=span
  cont ≫=span (λ mk →
...
check-typei Γ (Abs pi b {- All or Pi -} pi' x atk body) k = 
  ...
  where helper : maybe kind → 𝕃 tagged-val
        helper nothing = [ kind-data Γ star ]
        helper (just (Star _)) = [ kind-data Γ star ]
...
check-typei Γ (Mu pi pi' x knd body) (just k) =
  spanM-add (TpMu-span pi x knd body checking [] ) ≫span
  spanM-add (punctuation-span "Mu" pi (posinfo-plus pi 1)) ≫span
  check-kind Γ knd ≫span
  get-ctxt (λ Γ → 
   spanM-add (if conv-kind Γ knd k then
                TpMu-span pi x knd body checking [ kind-data Γ k ]
              else
                TpMu-span pi x knd body checking (mu-conv-error Γ x knd k [ kind-data Γ k ])) ≫span
  add-tk pi' x (Tkk knd) ≫=span λ mi →
  check-type Γ body (just knd) ≫span
  spanM-restore-info x mi)
    
check-typei Γ (Mu pi pi' x knd body) nothing =
  spanM-add (punctuation-span "Mu" pi (posinfo-plus pi 1)) ≫span
  check-kind Γ knd ≫span
  spanM-add (TpMu-span pi x knd body checking []) ≫span
  add-tk pi' x (Tkk knd) ≫=span λ mi →
...
check-typei Γ (TpArrow t1 _ t2) k = 
...
check-typei Γ (TpAppt tp t) k =
  check-type Γ tp nothing ≫=span cont'' ≫=spanr cont' k
...
check-typei Γ (Lft pi pi' X t l) k = 
  ...
  where cont : (outer : maybe kind) → kind → spanM (check-ret outer)
        cont nothing k = spanM-add (Lft-span pi X t synthesizing [ kind-data Γ k ]) ≫span spanMr (just k)
        cont (just k') k = 
          get-ctxt (λ Γ → 
            if conv-kind Γ k k' then 
              ...
              spanM-add (Lft-span pi X t checking ( error-data "The expected kind does not match the computed kind."
                                                 :: expected-kind Γ k' :: [ kind-data Γ k ])))
check-typei Γ (IotaEx pi ie pi' x (SomeType t1) t2) mk =
  spanM-add (Iota-span pi t2 (if-check-against-star-data Γ "A iota-type" mk)) ≫span
  check-typei Γ t1 (just star) ≫span
  add-tk pi' x (Tkt t1) ≫=span λ mi → 
  check-typei Γ t2 (just star) ≫span
  spanM-restore-info x mi ≫span
  return-star-when mk

check-typei Γ (IotaEx pi ie pi' x NoType t2) mk =
...
check-kind Γ (KndParens pi k pi') =
  ...
  check-kind Γ k
check-kind _ (Star pi) = spanM-add (Star-span pi checking)
check-kind Γ (KndVar pi x ys) =
