...
normalize : tpstate → renamectxt → bctxt → term → term
normalize s r b (Parens t) = normalize s r b t
normalize s r b (Lam x t) = 
  let x' = rename-away s b r x in
  (Lam x' (normalize s (renamectxt-insert r x x') (bctxt-add b x') t))
...
normalize s r b (App t1 t2) | Lam x t1' = 
  ...
  let r' = renamectxt-insert r x x' in
    normalize s r b (term-subst-term r' (rename-pred s b) (normalize s r b t2) x' t1')
