...
(defun cedille-mode-select-next(arg)
...
the parse tree, and updates the Cedille info buffer."
  ...
  (if (> arg 0)
	...
	(cedille-mode-select-next (- arg 1)))
      nil))

(defun cedille-mode-select-previous(arg)
...
the parse tree, and updates the Cedille info buffer."
  ...
  (if (> arg 0)
	...
	(cedille-mode-inspect)
	(cedille-mode-select-previous (- arg 1)))
      nil))
...
(defun cedille-mode-select-next-alt(arg)
	  ...
	  (let ((found (cl-find (se-mode-selected) se-mode-spans :test #'cedille-mode-select-next-alt-test)))
		...
		(message "No next span")
	      (progn (cedille-mode-select-span found)
		     (cedille-mode-inspect)))))
	(cedille-mode-select-next-alt (- arg 1)))
      nil))
      

(defun cedille-mode-select-previous-alt(arg)
	...
	(unless (se-mode-select (se-mode-previous))
	  (let ((found (cl-find (se-mode-selected) se-mode-spans
		...
		(message "No previous span")
	      (progn (cedille-mode-select-span found)
		     (cedille-mode-inspect)))))
	(cedille-mode-select-previous-alt (- arg 1)))
      nil))

(defun cedille-mode-select-parent(arg)
...
the parse tree, and updates the Cedille info buffer."
  ...
  (if (> arg 0)
	...
	(cedille-mode-inspect)
	(cedille-mode-select-parent (- arg 1)))
    nil))
    

(defun cedille-mode-select-first-child(arg)
...
containing point, and updates the Cedille info buffer."
  ...
  (if (> arg 0)
	...
	(cedille-mode-inspect)
	(cedille-mode-select-first-child (- arg 1)))
      nil))
