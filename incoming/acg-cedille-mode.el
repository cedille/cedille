...
(defun cedille-mode-get-context(tree)
  "Returns a list of symbols and their associated types/kinds"
  (let (output2)
    (while tree
      (let* ((data (se-term-data (car (se-node-children (car tree)))))
	     (symbol (cdr (assoc 'symbol data)))
	     (kind (cdr (assoc 'kind data)))
	     (type (cdr (assoc 'type data))))
	(if (and symbol (or type kind))
	    (setq output2 (cons (cons symbol (concat kind type)) output2))))
      (setq tree (cdr tree)))
    output2))

(defun cedille-mode-format-context(tree)
  "Formats the context as text for display"
  (let ((output1 "")
	(context (cedille-mode-get-context tree)))
    (while context
      (let* ((head (car context))
	     (symbol (car head))
	     (value (cdr head)))
	(setq output1 (concat symbol ":  " value "\n" output1))
	(setq context (cdr context))))
    output1))
      
(defun cedille-mode-context()
  "Shows the context for the current node"
  (interactive)
  (if se-mode-selected
      (let* ((b (cedille-info-buffer))
	     (d (se-term-to-json (se-mode-selected)))
